...
public class WB_Play6 extends CCColorLayer {
...
//	private int _ChangeImage_Delay = -1;// ͼƬ�ı���ʱ
	...
	private int Wudi_Num=1;
	private int Bomb_Num =1;// ��ը������
	private int Hp_Num=1;
	private int Prop_Down_Time = 30;// ���ӵ��ͺ�ը���������
	private int DripDown_Time = 8;// �л��������ٶ�
	private int Capa=23;//ˮ��
...
//	private boolean _Invincible = false;//�޵�
   //��������ֵ�ж��Ƿ���һ������ֵ�ڽ��ܹ�20% 4%��ˮ���ͷ�
    ...
    protected WB_Play6(ccColor4B color,int a,int b,int c,int d,int e,int f) {
		super(color);
...
//		this.schedule("AddPlay", 0.2f);
		...
		this.schedule("AddIncDecDown",3.0f);
...
//		}
		//��������
		if(randomValue>0&&randomValue<=2&&Season==4)
		{
			wb_drip.setLift(_Small_life);
			wb_drip.setMax_Life(_Small_life);
			wb_drip.setCCSprite(_SmallDrip_Path);
		...
		//��������
		if(randomValue>=5&&randomValue<=7&&Season==1)
		{
			wb_drip.setLift(_Small_life);
			wb_drip.setMax_Life(_Small_life);
			wb_drip.setCCSprite(_SmallDrip_Path);
  ...
  /**
	...
	private void Down(WB_Sprite wb_drip, int i, float y) {
		...
		CCFiniteTimeAction fs_timeAction = CCMoveTo.action(wb_drip
				...
				-(wb_drip.getCCSprite().getContentSize().height / 2)));// ʱ�����ƶ�
		CCCallFuncN fs_Over = null;
		if (i == 1)
			fs_Over = CCCallFuncN.action(this, "Drip_Over");
...
//	}
	private void setDownDistance(WB_Sprite wb_drip)
	{
		...
		case 1:
			...
			break;
		case 2:
			_Down_Distance=_WinSize.getHeight()*3/4;
		    break;
		case 3:
			_Down_Distance=_WinSize.getHeight()*3/4;
			break;
		case 4:
			_Down_Distance=_WinSize.getHeight()*3/5;
			break;
		}
	}
...
//			WB_Sprite drip = (WB_Sprite) _BigDrip.get(j);
//			CGRect Rect2 = drip.getCCSprite().getBoundingBox();
//               if (CirCleinterRect(Rect_Ball.size.width/2,Rect_Ball.origin.x+Rect_Ball.size.width/2,Rect_Ball.origin.y+Rect_Ball.size.height/2,
//					Rect2.origin.x,Rect2.origin.y,Rect2.size.width,Rect2.size.height)) {
//                 non_drip=0;
